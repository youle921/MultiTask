...
public class Distance {
	...
	public double[][] distanceMatrix(SolutionSet solutionSet, Solution objSet) {
		...
		for (int i = 0; i < solutionSet.size(); i++) {
			...
			for (int j = i + 1; j < solutionSet.size(); j++) {
				solutionJ = solutionSet.get(j);
				distance[i][j] = this.distanceBetweenObjectives(solutionI, solutionJ, objSet);
				distance[j][i] = distance[i][j];
			} // for
		} // for
	...
	public double[][] distanceMatrix(SolutionSet solutionSet, boolean[] isChosen) {
		...
		for (int i = 0; i < solutionSet.size(); i++) {
			...
			for (int j = i + 1; j < solutionSet.size(); j++) {
				solutionJ = solutionSet.get(j);
				distance[i][j] = this.distanceBetweenObjectives(solutionI, solutionJ, isChosen);
				distance[j][i] = distance[i][j];
			} // for
		} // for
	...
	public double distanceBetweenObjectives(Solution solutionI, Solution solutionJ,  Solution objSet) {
		...
		// -> Calculate the euclidean distance
		for (int nObj = 0; nObj < solutionI.getNumberOfObjectives(); nObj++) {
			boolean flag = ((Binary)(objSet.getDecisionVariables()[0])).getIth(nObj);
			if (!flag)
				continue;
			diff = solutionI.getObjective(nObj) - solutionJ.getObjective(nObj);
			distance += Math.pow(diff, 2.0);
	...
	public void crowdingDistanceAssignment(SolutionSet solutionSet, int nObjs, Solution objSet) {
		int size = solutionSet.size();
		...
		for (int i = 0; i < nObjs; i++) {
			// Sort the population by Obj

			boolean chosen = ((Binary) objSet.getDecisionVariables()[0]).getIth(i);
			if (!chosen)
				continue;
	...
	public void crowdingDistanceAssignment(SolutionSet solutionSet, int nObjs, boolean[] isChosen) {
		int size = solutionSet.size();
...
//				System.out.println("");
	...
	public double distanceBetweenObjectivesSDE(Solution ind1, Solution ind2, Solution objSet) {
		...
		for (i = 0; i < nobj; i++) {
			boolean flag = ((Binary)(objSet.getDecisionVariables()[0])).getIth(i);
			if (!flag)
				continue;
			if (ind1.getObjective(i) < ind2.getObjective(i))
				d += (ind2.getObjective(i) - ind1.getObjective(i))
		...
		for (int i = 0; i < solutionSet.size(); i++) {
			for (int j = 0; j < solutionSet.size(); j++) {
				if (j == i)
					...
					distance[i][j] = this.distanceBetweenObjectivesSDE(
							solutionSet.get(i), solutionSet.get(j), isChosen);
			}
		} // for
		...
		for (int i = 0; i < solutionSet.size(); i++) {
			for (int j = 0; j < solutionSet.size(); j++) {
				if (j == i)
					...
					distance[i][j] = this.distanceBetweenObjectivesSDE(
							solutionSet.get(i), solutionSet.get(j), objSet);
			}
		} // for
