 ...
 * solutions.
 */
public class MyTwoPointCrossover extends Crossover {
	...
	private double twoPointAlpha = 0.5;
	...
	public MyTwoPointCrossover(HashMap<String, Object> parameters) {
		super(parameters);
		...
		if (parameters.get("TwoPointAlpha") != null)
			twoPointAlpha = (Double) parameters.get("TwoPointAlpha");
	...
	public Solution[] doCrossover(double probability, Solution parent1, Solution parent2) throws JMException {
		...
		if (PseudoRandom.randDouble() <= probability) {
				...
				int nOfSwapedVal = (int)(twoPointAlpha*numberOfVariables);

				int p1 = PseudoRandom.randInt(0, numberOfVariables-1);

				int l = PseudoRandom.randInt(0,nOfSwapedVal);

				int p2 = (p1 + l) % numberOfVariables;


				int startPoint = Integer.min(p1, p2);
				int endPoint = Integer.max(p1, p2);
				for(int i=startPoint; i <= endPoint;i++) {
					double one = offSpring[0].getDecisionVariables()[i].getValue();;
					double two = offSpring[1].getDecisionVariables()[i].getValue();;

					offSpring[0].getDecisionVariables()[i].setValue(two);
					offSpring[1].getDecisionVariables()[i].setValue(one);
