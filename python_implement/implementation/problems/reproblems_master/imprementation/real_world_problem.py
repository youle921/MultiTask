...
class RE_base_class:
    ...
    def set_reference_point(self, name):

        self.IGD_ref = np.loadtxt(
            f'{self.current_path}/../approximated_Pareto_fronts/reference_points_{name}.dat')
...
# apply transformation(normalize + shift + rotate)
    ...
    def calc_IGD(self, objective):
        ...
        dist = np.sqrt(
            np.sum((self.IGD_ref[:, None, :] - objective[None, :, :])**2, axis=2))
